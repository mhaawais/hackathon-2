...
# Get project root (where .specify/ lives, not necessarily git root)
SCRIPT_DIR="$(CDPATH="" cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(CDPATH="" cd "$SCRIPT_DIR/../../.." && pwd)"
SPECS_DIR="$PROJECT_ROOT/specs"

# Check for template (try both locations relative to project root)
TEMPLATE_PATH=""
if [[ -f "$PROJECT_ROOT/.specify/templates/phr-template.prompt.md" ]]; then
  TEMPLATE_PATH="$PROJECT_ROOT/.specify/templates/phr-template.prompt.md"
elif [[ -f "$PROJECT_ROOT/templates/phr-template.prompt.md" ]]; then
  TEMPLATE_PATH="$PROJECT_ROOT/templates/phr-template.prompt.md"
else
...
case "$STAGE" in
  constitution)
    ...
    PROMPTS_DIR="$PROJECT_ROOT/history/prompts/constitution"
  ...
  spec|plan|tasks|red|green|refactor|explainer|misc)
    ...
    PROMPTS_DIR="$PROJECT_ROOT/history/prompts/$FEATURE"
  ...
  general)
    ...
    PROMPTS_DIR="$PROJECT_ROOT/history/prompts/general"
